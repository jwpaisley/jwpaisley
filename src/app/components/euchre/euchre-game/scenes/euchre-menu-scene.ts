...
import { EuchreGameUtility } from "../utils/euchre-game-util";

const JACKS: EuchreCardData[] = [
    { rank: 'J', suit: 'clubs' },
    { rank: 'J', suit: 'hearts' },
    { rank: 'J', suit: 'spades' },
...
export class EuchreMenuScene extends Phaser.Scene {
    ...
    get buttonsY(): number {
        if (EuchreGameUtility.screenIsPortrait(this)) {
    ...
    startTitleAnimation() {
        ...
        this.tweens.add({
            targets: this.title,
            y: this.titleY - (10 * (this.scale.height / 800)),
            duration: 1000,
            ease: 'Sine.easeInOut',
            yoyo: true,
            repeat: -1
    ...
    addJacksToMenuScene() {
        ...
        JACKS.forEach((jackData, index) => {
            ...
            card.setData('cardFace', EuchreCardUtility.getFrame(jackData));
